...
/// The storage a run writes into, placed outside any stack frame.
var storage: chunk_gate.Storage = .zeroed;
...
pub fn parse(arguments: []const []const u8) error{Usage}!Command {
    ...
    if (std.mem.eql(u8, arguments[0], "--chunk-seed")) {
        return .{ .chunk_seed = try parse_seed(value orelse return error.Usage) };
    }
    if (std.mem.eql(u8, arguments[0], "--chunk-gate")) {
        const text = value orelse return .{ .chunk_gate = constants.gate_seeds_default };
        const seeds = std.fmt.parseInt(u64, text, count_radix) catch return error.Usage;
        return .{ .chunk_gate = seeds };
    }
    return error.Usage;
}
...
fn parse_seed(text: []const u8) error{Usage}!u64 {
    const digits = if (std.mem.startsWith(u8, text, hex_prefix)) text[hex_prefix.len..] else text;
    return std.fmt.parseInt(u64, digits, seed_radix) catch error.Usage;
}

fn chunk_seed(seed: u64) !void {
    const result = chunk_gate.run_seed(&storage, seed) catch |failure| {
...
fn chunk_gate_seeds(seeds: u64) !void {
    ...
    chunk_gate.run_gate(&storage, seeds, &census, &failed_seed) catch |failure| {
