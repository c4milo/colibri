//! The stream the connection check feeds (design §8 step 4): the octets a client sends a server.
//!
...
const data_payload: [constants.connection_check_data_len_max]u8 = @splat('d');
...
const initial_window_sizes = [_]u32{
    0,
    1,
    constants.connection_check_window_size_small,
    h2_constants.initial_window_size_initial,
};

pub const Plan = struct {
    frames: [constants.connection_check_frames_max]Frame,
    count: u32,
    ...
    pub fn draw(random: *Random) Plan {
        var plan: Plan = .{
            ...
            .count = @intCast(random.between(1, constants.connection_check_frames_max)),
        ...
        if (random.below(constants.connection_check_refusal_one_in) == 0) {
            const refusals = std.enums.values(Refusal);
            plan.refusal = refusals[random.below(refusals.len)];
        }
        assert(plan.count >= 1 and plan.count <= constants.connection_check_frames_max);
...
const Opened = struct {
    ...
    fn init(random: *Random) Opened {
        const drawn = random.below(constants.connection_check_first_stream_ids);
    ...
    fn open(opened: *Opened, random: *Random) u32 {
        ...
        opened.count += 1;
        const steps = random.between(1, constants.connection_check_stream_id_steps);
...
fn draw_increment(random: *Random) u32 {
    ...
    return @intCast(random.between(1, constants.connection_check_increment_max));
}
...
fn draw_request(random: *Random, opened: *Opened, kind: Kind) Frame {
    ...
    return .{
        ...
        .fragments = if (continued) @intCast(random.between(
            constants.connection_check_fragments_min,
            constants.connection_check_fragments_max,
        )) else 1,
    };
}
...
fn draw_stream_frame(random: *Random, opened: *Opened, kind: Kind) Frame {
    ...
    return .{
        ...
        .stream_id = id,
        .end_stream = draw_flag(random),
        .data_len = @intCast(random.between(0, constants.connection_check_data_len_max)),
    };
}
...
fn write_continued(drawn: *const Frame, output: *Writer) core.writer.Error!void {
    const block = block_of(drawn);
    assert(drawn.fragments >= constants.connection_check_fragments_min);
    assert(drawn.fragments <= constants.connection_check_fragments_max);
...
comptime {
    ...
    assert(preface_len <= constants.connection_check_preface_len_max);
    ...
    const data_len = h2_constants.frame_header_len + constants.connection_check_data_len_max;
    const continued_len = constants.connection_check_fragments_max * h2_constants.frame_header_len +
        request_block_authority.len;
    assert(data_len <= constants.connection_check_frame_len_max);
    assert(continued_len <= constants.connection_check_frame_len_max);
    ...
    const refusal_room = constants.connection_check_refusal_frames_max *
        constants.connection_check_frame_len_max;
    ...
    assert(constants.connection_check_frames_max < h2_constants.rst_stream_rate_max);
    // The CONTINUATION frames one request is cut into stay under §6.10's bound (invariant 14).
    assert(constants.connection_check_fragments_max <= h2_constants.continuation_count_max + 1);
}

const testing = std.testing;

/// Where the tests write a plan's stream, placed outside any stack frame. Test-only.
var test_stream: [constants.connection_check_stream_len_max]u8 = @splat(0);
...
test "every plan's stream opens with the preface and fits the named length" {
    ...
    for (0..constants.check_seeds_default) |seed| {
        ...
        try testing.expect(stream.len <= constants.connection_check_stream_len_max);
        if (plan.refusal != null) refused += 1;
    }
    try testing.expect(refused > 0 and refused < constants.check_seeds_default);
}
...
test "a request cut into fragments writes the same field block as one written whole" {
    ...
    for (2..constants.connection_check_fragments_max + 1) |fragments| {
        const cut: Frame = .{
